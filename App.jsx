...
const sanitizeFusionMap = (map) => {
  ...
  Object.entries(map || {}).forEach(([source, target]) => {
    const cleanedSource = cleanName(source);
    const cleanedTarget = cleanName(target);
    if (!cleanedSource || !cleanedTarget) return;
    sanitized[cleanedSource] = cleanedTarget;
  });
  return sanitized;
};
...
export default function Dashboard() {
  ...
  const [hiddenCdIds, setHiddenCdIds] = useState(new Set());
  ...
  const [searchCAI, setSearchCAI] = useState('');
  ...
  const autoNameMap = useMemo(() => {
    ...
    rawData.forEach((row) => {
      ...
      [row[16], row[17], row[18]].forEach((name) => {
        ...
        const isExistingMonocase = existing === existing.toUpperCase() || existing === existing.toLowerCase();
        const isCandidateMonocase = cleaned === cleaned.toUpperCase() || cleaned === cleaned.toLowerCase();
        if (isExistingMonocase && !isCandidateMonocase) {
          mapped[key] = cleaned;
        }
      });
    });
  ...
  const sanitizedNameFusions = useMemo(() => sanitizeFusionMap(nameFusions), [nameFusions]);

  const fusionLookups = useMemo(() => {
    const normalized = {};
    Object.entries(sanitizedNameFusions).forEach(([source, target]) => {
      const key = normalizeNameKey(source);
      if (key) normalized[key] = target;
    });
    return { direct: sanitizedNameFusions, normalized };
  }, [sanitizedNameFusions]);

  const updateNameFusions = (updater) => {
    setNameFusions((prev) => {
  ...
  const allUniqueNames = useMemo(() => {
    ...
    Object.values(autoNameMap).forEach((name) => unique.add(name));
    Object.keys(fusionLookups.direct).forEach((name) => unique.add(name));
    Object.values(fusionLookups.direct).forEach((name) => unique.add(name));
    return Array.from(unique).sort((a, b) => a.localeCompare(b, 'fr', { sensitivity: 'base' }));
  }, [autoNameMap, fusionLookups]);

  const getNormalizedName = (rawName) => {
    const cleaned = cleanName(rawName);
    if (!cleaned) return null;
    if (fusionLookups.direct[cleaned]) return fusionLookups.direct[cleaned];
    const key = normalizeNameKey(cleaned);
    if (key && fusionLookups.normalized[key]) return fusionLookups.normalized[key];
    if (key && autoNameMap[key]) return autoNameMap[key];
    return cleaned;
  };
  ...
  const cdData = useMemo(() => {
    ...
    for (const row of rawData) {
      ...
      const tempsD1 = parseFloat(row[86]) || 0;
      ...
      // Qualité avec Niv1, Niv2, Niv3
      let qualite = 'Niv1';
      let qualiteInfo = null;
      if (row[68]) { 
        qualite = 'Niv3'; 
        qualiteInfo = typeof row[68] === 'string' ? row[68] : "Pas d'informations"; 
      }
      else if (row[69]) { 
        qualite = 'Niv2'; 
        qualiteInfo = typeof row[69] === 'string' ? row[69] : "Pas d'informations"; 
      }
      else if (row[70]) {
        // Niv1 avec info
        qualiteInfo = typeof row[70] === 'string' ? row[70] : null;
      }
      
      // Extraire l'heure au format "6H" (sans minutes)
      let heureCD = null;
      if (row[19]) {
        try {
          const heureData = row[19] instanceof Date ? row[19] : new Date(row[19]);
          if (heureData && !isNaN(heureData.getTime())) {
            const heures = heureData.getHours();
            heureCD = `${heures}H`;
          }
        } catch (e) {
          heureCD = null;
      ...
      const pannes = parseFloat(row[111]) || 0;
      data.push({ 
        id: cdId++, 
        date: dateStr, 
        week: row[3] || null, 
        ...
        conf2, 
        tempsD1, 
        tempsD1Net: parseFloat(row[112]) || 0, 
        qualite, 
        qualiteInfo, 
        isPanne: pannes > 0, 
    ...
    return data;
  }, [rawData, autoNameMap, fusionLookups]);
  ...
  const feedbackStats = useMemo(() => {
    ...
    const visibleCDs = feedbackFilteredCDs.filter(cd => !hiddenCdIds.has(cd.id));
    ...
    // Calculer avgTimeNet en excluant les D1 NET = 0
    const cdWithNet = visibleCDs.filter(cd => cd.tempsD1Net > 0);
    const avgTimeNet = cdWithNet.length > 0 
      ? cdWithNet.reduce((s, cd) => s + cd.tempsD1Net, 0) / cdWithNet.length 
      : 0;
  ...
  }, [feedbackFilteredCDs, hiddenCdIds]);
        ...
        {activeTab === 'historique' && (
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="mb-4 space-y-3">
              ...
              <div className="flex items-center gap-3">
                <label className="text-sm font-medium text-slate-700">Rechercher par CAI :</label>
                <input 
                  type="text"
                  value={searchCAI}
                  onChange={(e) => setSearchCAI(e.target.value)}
                  placeholder="Entrez un CAI..."
                  className="flex-1 px-4 py-2 border-2 border-slate-300 rounded-lg text-sm hover:border-blue-500 focus:border-blue-500 focus:outline-none transition-colors"
                />
                {searchCAI && (
                  <button 
                    onClick={() => setSearchCAI('')}
                ...
                <tbody>
                  {sortedHistoryCdData.filter(cd => {
                    ...
                    <tr key={cd.id} onClick={() => setSelectedCD(cd)} className="border-b hover:bg-slate-50 cursor-pointer">
                      ...
                      <td className="py-2 px-3 text-xs font-mono text-blue-600">{cd.cai || '-'}</td>
            ...
            <p className="text-sm text-slate-600 mt-4 text-center">{sortedHistoryCdData.filter(cd => !searchCAI || (cd.cai && cd.cai.toLowerCase().includes(searchCAI.toLowerCase()))).length} CD affichés</p>
          </div>
        )}
                ...
                {feedbackStats && (
                  <div className="bg-white rounded-xl shadow-sm p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {feedbackStats.hidden > 0 && <span className="text-sm bg-slate-200 text-slate-700 px-3 py-1 rounded-full font-medium">{feedbackStats.hidden} CD masqué{feedbackStats.hidden > 1 ? 's' : ''}</span>}
                    </div>
                    ...
                    <div className="overflow-auto max-h-[500px]">
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-slate-50 border-b-2">
                          <tr><th className="text-left py-3 px-3">Date</th><th className="text-center py-3 px-3">Rôle</th><th className="text-center py-3 px-3">D1</th><th className="text-center py-3 px-3">D1 NET</th><th className="text-center py-3 px-3">Qualité</th><th className="text-center py-3 px-3">CQ</th><th className="text-left py-3 px-3">Info Qualité</th><th className="text-center py-3 px-3">Masquer</th></tr>
                        </thead>
                        <tbody>
                          {feedbackFilteredCDs.map((cd) => {
                            const isConf1 = cd.conf1 === feedbackOperator;
                            const isHidden = hiddenCdIds.has(cd.id);
                            const hasCQ = cd.cqCW || cd.cqCX || cd.cqCY;
                            return (
                              <tr 
                                key={cd.id} 
                                className={`border-b cursor-pointer ${isHidden ? 'opacity-40 bg-slate-100' : 'hover:bg-blue-50'}`}
                              ...
                              >
                                ...
                                <td className="py-3 px-3 text-center">
                                  {cd.tempsD1Net > 0 ? (
                                    <span className="font-semibold text-sm text-blue-700">{cd.tempsD1Net}h</span>
                                  ) : (
                                ...
                                <td className="py-3 px-3 text-xs max-w-xs truncate">
                                  {cd.qualiteInfo ? (
                                    <span className={`${cd.qualite === 'Niv2' ? 'text-orange-700' : cd.qualite === 'Niv3' ? 'text-red-700' : 'text-green-700'} font-medium`}>{cd.qualiteInfo}</span>
                                  ) : (
                                ...
                                <td className="py-3 px-3 text-center">
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); toggleHideCd(cd.id); }} 
                                    className={`px-3 py-1 rounded text-xs font-medium transition-colors ${isHidden ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200'}`} 
                                    title={isHidden ? 'Réintégrer dans les stats' : 'Exclure des stats'}
                                  >
                                    {isHidden ? '👁️ Afficher' : '🚫 Masquer'}
                                  </button>
                                </td>
                              </tr>
                            );
              ...
              <div className="border-t pt-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="bg-blue-50 p-2 rounded-lg text-center">
                    <p className="text-xs font-medium mb-1">D1 NET</p>
                    {selectedCD.tempsD1Net > 0 ? (
                      <p className="text-xl font-bold text-blue-700">{selectedCD.tempsD1Net}h</p>
                    ) : (
              ...
              <div className="border-t pt-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-50 p-2 rounded">
                    <p className="text-xs text-slate-600 mb-0.5">CAI</p>
                    <p className="text-sm font-bold text-blue-600 font-mono">{selectedCD.cai}</p>
                  </div>
                  <div className="bg-slate-50 p-2 rounded">
                    <p className="text-xs text-slate-600 mb-0.5">Heure CD</p>
                    <p className="text-sm font-bold text-purple-600">{selectedCD.heureCD}</p>
                  </div>
                  <div className="bg-slate-50 p-2 rounded">
                  ...
                  {selectedCD.aleasCD && (
                    <div className="bg-amber-50 p-2 rounded col-span-2">
                      <p className="text-xs text-amber-700 font-medium mb-0.5">Aléas CD</p>
                      <p className="text-sm font-bold text-amber-900">{selectedCD.aleasCD}</p>
                    </div>
                  )}
                </div>
              </div>
